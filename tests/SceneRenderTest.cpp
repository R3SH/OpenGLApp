...
namespace test {
	SceneRenderTest::SceneRenderTest()
	{
		...
		//Shaders init
		m_Shader = std::make_unique<Shader>("res/shaders/SceneRender.shader");
		...
		m_LightBulb = new Model(modelSrc + "\\res\\models\\lightbulb.obj");
		//m_Model = new Model("res/models/AK74M.obj");

		m_GlockPos = glm::vec3(0.5f, 0.0f, 0.0f);
		m_GlockHandsPos = glm::vec3(0.1f, -0.1f, 2.5f);
		m_ModelMovSpeed = 0.1f;
	...
	GLuint SceneRenderTest::LoadTexture(const std::string& texturePath)
	{
	...
	void SceneRenderTest::OnUpdate(float deltaTime)
	{
	...
	void SceneRenderTest::OnRender()
	{
		...
		//GLOCK17
		model = glm::mat4(1.0f);
		model = glm::translate(model, m_GlockPos);
	...
	// ---------------------------------------------------------------------------------------------
	void SceneRenderTest::framebuffer_size_callback(GLFWwindow* window, int width, int height)
	{
	...
	// ---------------------------------------------------------------------------------------------------------
	void SceneRenderTest::ProcessInput()
	{
	...
	// -------------------------------------------------------
	void SceneRenderTest::mouse_callback(GLFWwindow* window, double xposIn, double yposIn)
	{
	...
	// ----------------------------------------------------------------------
	void SceneRenderTest::scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
	{
